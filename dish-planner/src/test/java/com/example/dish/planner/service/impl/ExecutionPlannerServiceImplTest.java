...
class ExecutionPlannerServiceImplTest {
    ...
    void shouldBuildSingleStepPlanForChatIntent() {
        ExecutionPlannerServiceImpl planner = new ExecutionPlannerServiceImpl(new ExecutionPlanFactory());
        ...
        Assertions.assertEquals(ExecutionModes.SINGLE, result.plan().metadata().get("executionMode"));
        ...
        Assertions.assertEquals(AgentTargets.CHAT, node.target());
    }
    ...
    void shouldBuildTwoStepPlanForDishIntent() {
        ...
        Assertions.assertTrue(result.success());
        Assertions.assertEquals(ExecutionModes.SERIAL, result.plan().metadata().get("executionMode"));
        ...
        Assertions.assertEquals(List.of(AgentTargets.DISH_KNOWLEDGE, AgentTargets.CHAT), targets);
    ...
    void shouldBuildTwoStepPlanForWorkOrderIntent() {
        ...
        Assertions.assertTrue(result.success());
        List<String> targets = result.plan().nodes().stream().map(ExecutionNode::target).toList();
        Assertions.assertEquals(List.of(AgentTargets.WORK_ORDER, AgentTargets.CHAT), targets);
        Assertions.assertEquals(1, result.plan().edges().size());
    }
}
