...
| `adr/` | 难逆转、跨阶段、需要背景的持久决策 | 临时想法、常规实现取舍 |
| `discovery/` | 上游探索输入与已消费归档 | 当前执行态、正式 phase 需求或技术契约 |
| `interview/` | 面试展示和讲解补充材料 | 当前正式真相层 |
| `openapi/` | 接口契约资产 | 产品定位、phase 规划、执行态 |
| `templates/` | 项目内可复用文档骨架 | 当前项目状态、真实证据 |

## 跨阶段技术辅助文档

像 [PERFORMANCE_OPTIMIZATION.md](PERFORMANCE_OPTIMIZATION.md) 这类跨阶段技术策略文档，默认视为辅助层：

- 可以为多个 phase 的技术设计提供输入
- 不直接拥有某个 phase 的技术契约
- 不替代 `docs/technical/` 下的阶段技术设计
- 只有当当前任务显式涉及性能、缓存、容量或延迟预算时，才按需读取
